...
const AddNews = () => {
  ...
  const uploadImage = async () => {
    ...
    return { imageName: formData.title, imagePaths: allImagePaths };
  };

  const handleSubmit = async () => {
    setLoad(true);
    if (!validateForm()) { setLoad(false); return; }
    try {
  ...
  if (load) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 backdrop-blur-sm">
        <img src={loading_gif} alt="Loading..." className="w-12 h-12 sm:w-16 sm:h-16" />
    ...
    <div className="flex w-full flex-col items-center justify-center p-4 sm:p-6 lg:p-10">
      <Card extra="w-full max-w-3xl p-6 sm:p-8 shadow-xl rounded-2xl bg-white dark:bg-navy-700">
        ...
        <div className="mb-4">
          ...
          <div className="mt-2">
            <label className="inline-block cursor-pointer rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600">
              Choose Files
              <input
                type="file"
                accept="image/*"
